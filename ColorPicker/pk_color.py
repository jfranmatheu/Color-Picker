from . widget import SubLayout, Anchor, Vector
from . slider_2d import Slider2D
...
from . slider_rng import SliderRingDot
cache = {}


class PkColorQuad(Slider2D):
    def __init__(self, _brush, layout: SubLayout = None, anchor: Anchor = None, draw_callback: callable = None) -> object:
        ...
        self.set_slider_x(self._color, 's', 0.00001)
        self.set_slider_y(self._color, 'v', 0.00001)
        self._live_update = True

    def get_pos_size(self) -> (Vector, Vector):
        return self.dot_center_center(), self.size

    def set_slider_x(self, data_source, attr_source: str, min_value: float = 0, max_value: float = 1) -> None:
        super().set_slider_x(data_source, attr_source, min_value, max_value)
        self._handle_pos.x = self.dot_center_center().x + self._factor_x * self.size.x # pow(self._factor_x, .454545)

    def set_slider_y(self, data_source, attr_source: str, min_value: float = 0, max_value: float = 1) -> None:
        super().set_slider_y(data_source, attr_source, min_value, max_value)
...
class ToSlicePkColorQuad(PkColorQuad):
    def __init__(self, _brush, layout: SubLayout = None, anchor: Anchor = None, draw_callback: callable = None) -> object:
        super().__init__(_brush, layout, anchor, draw_callback)
    ...
    def update_slicing(self) -> None:
        self.set_slider_x(self._data_x, self._attr_x, max(self.slices.min.x, 0.0001), self.slices.max.x)
        self.set_slider_y(self._data_y, self._attr_y, max(self.slices.min.y, 0.0001), self.slices.max.y)
        self.update_values()
...
class SlicedPkColorQuad(PkColorQuad):
    ...
    def on_cancel(self) -> None:
        ...
        if self._on_hover_slicer == MIN_X: self._slices.min.x = self._prev_slice_factor
        elif self._on_hover_slicer == MIN_Y: self._slices.min.y = self._prev_slice_factor
        elif self._on_hover_slicer == MAX_X: self._slices.max.x = self._prev_slice_factor
        elif self._on_hover_slicer == MAX_Y: self._slices.max.y = self._prev_slice_factor
        
        Cursor.set_icon(None, CursorIcon.DEFAULT)

    def on_click(self) -> None:
        super().on_click()
        if self._on_hover_slicer:
            if self._on_hover_slicer in {MIN_X, MAX_X}:
                Cursor.set_icon(None, CursorIcon.MOVE_X)
                if self._on_hover_slicer == MIN_X: self._prev_slice_factor = self._slices.min.x
                else: self._prev_slice_factor = self._slices.max.x
            elif self._on_hover_slicer in {MIN_Y, MAX_Y}:
                Cursor.set_icon(None, CursorIcon.MOVE_Y)
                if self._on_hover_slicer == MIN_Y: self._prev_slice_factor = self._slices.min.y
    ...
    def on_slide(self, mouse: Vector) -> None:
        ...
        if self._on_hover_slicer == MIN_X:
...
class PkColorCircle(SliderRadialDot):
    def __init__(self, _brush, layout: SubLayout = None, anchor: Anchor = None, draw_callback: callable = None) -> object:
        super().__init__(layout, anchor, draw_callback)
        self._color = _brush.color
        self.set_pos(self.pos.x+self.size.x/2, self.pos.y+self.size.y/2)
        minimo = min(self.size.x, self.size.y)
        self.set_size(minimo, minimo)
        self.set_slider_angle(self._color, 'h', 0.0, 1.0)
        self.set_slider_distance(self._color, 's', 0.0, 1.0)
        self._live_update = True
...
class PkColorRing(SliderRingDot):
    def __init__(self, _brush, layout: SubLayout = None, anchor: Anchor = None, thickness: float = 20, draw_callback: callable = None) -> object:
        super().__init__(layout, anchor, thickness, draw_callback)
        self._color = _brush.color
        self.set_pos(self.pos.x+self.size.x/2, self.pos.y+self.size.y/2)
        minimo = min(self.size.x, self.size.y)
        self.set_size(minimo, minimo)
        self.set_slider_value(self._color, 'h', 0.0, 1.0)
        self._live_update = True
        side = self.size.x
    ...
    def set_inner_widget(self, widget):
        ...
        widget.pos = c + Vector((-3, 3)) # rotate(c, down_over_thicknes, -45) 
        side = (self.size.x - self._thickness * 4) * .9125
        widget.size = Vector((side, side)) + Vector((-10, -10))
        if isinstance(widget, ToSlicePkColorQuad):
