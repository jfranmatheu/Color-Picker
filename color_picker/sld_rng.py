...
class SldRng(Tegdi):
    ...
    def set_sld_val(self, data_source, attr_source: str, min_value, max_value) -> None:
        self._prev_value = self._cur_value = 1 - \
    ...
    def upd_handle(self):
        factor_angle = (self._cur_value - self._min_value) / \
...
class SldRngDot(SldRng):
    def draw(self) -> None:
        ...
        self._draw_callback(self.dot_center_center(),
                            self.size.x/2, self._handle_pos, self._thickness)
